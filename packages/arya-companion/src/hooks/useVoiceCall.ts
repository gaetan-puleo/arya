...
const SPEECH_RMS = 0.015; // RMS above this = voice present. Tuned to real device levels @16k: measured voice ≈0.02–0.05, silence ≈0.001–0.01.
	...
	const onChunk = useCallback(
		async (event: AudioDataEvent): Promise<void> => {
			...
			for (let i = 0; i < WAVE_BARS; i++) {
				const lvl = Math.max(0, buckets[i] - WAVE_NOISE_FLOOR);
			...
			if (dbgRef.current === 0) {
				console.log(
					`[voice] rms=${rms.toFixed(3)} voiced=${voiced} voicedMs=${voicedMsRef.current | 0} segMs=${segMs | 0} silence=${silence | 0} spoken=${hasSpoken}`,
				);
			}
