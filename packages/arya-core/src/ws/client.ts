...
import type { AgentSession, AgentSessionEvent, SessionRecord } from 'mu-coding';
import type { ApprovalManager, PendingApproval } from 'mu-coding';
import type { SubAgentRegistry, SubAgentResult, SubAgentRun } from 'mu-coding';
import type { ChatFeatures, ChatHost, ModelInfo } from 'mu-coding';
import { createWsClient } from './ws-client';
import { partsToAttachments, textOf, type WireMessage } from './wire';
import type { SubAgentEventWire, WireAgent, WireCommand, WirePanelSection, WsOutbound } from './protocol';
...
export interface ConnectHarnessOptions {
  ...
  banner?: string;
  minimal?: boolean;
  /** Host application version (arya), surfaced to the client's panel footer. */
  version?: string;
...
export async function connectHarness(opts: ConnectHarnessOptions): Promise<RemoteHarness> {
  ...
  const panelListeners = new Set<(sections: WirePanelSection[]) => void>();
  ...
  // Settle every request-keyed waiter so callers never hang when the socket drops
  // (or the server tears the connection down, e.g. an oversized frame).
  const failPendingWaiters = (message: string): void => {
    const err = new Error(message);
    for (const w of subagentWaiters.values()) w.reject(err);
    subagentWaiters.clear();
  };
  client.onClose(() => failPendingWaiters('connection lost'));
  ...
  const unsub = client.on((frame: WsOutbound) => {
    switch (frame.type) {
      ...
      case 'model_loading':
        ...
        return;
      case 'panel:update':
        for (const l of [...panelListeners]) l(frame.sections);
        return;
      case 'models:listed': {
